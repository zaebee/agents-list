...
class SlackIntegration:
    ...
    async def handle_slash_command(self, ack, command, respond):
        ...
        try:
            ...
            if not args or args[0] == "help":
                ...
                await respond(
                    f"Unknown command: {args[0]}. Use `/ai-crm help` for available commands."
                )
    ...
    async def handle_task_command(self, args: List[str], command: Dict, respond):
        ...
        if subcommand == "create":
            ...
            if len(args) > 1:
                ...
                await respond(
                    "Please provide a task ID: `/ai-crm task status <task_id>`"
                )
        else:
    ...
    async def show_task_creation_modal(self, command: Dict):
        ...
        await self.client.views_open(trigger_id=command["trigger_id"], view=modal_view)
    ...
    async def handle_task_creation_modal(self, ack, body, view):
        ...
        try:
            ...
            priority = (
                values["task_priority"]["task_priority"]["selected_option"]["value"]
                if values["task_priority"]["task_priority"]["selected_option"]
                else "medium"
            )

            analysis_options = []
            if (
                values.get("analysis_options")
                and values["analysis_options"]["analysis_options"]["selected_options"]
            ):
                analysis_options = [
                    opt["value"]
                    for opt in values["analysis_options"]["analysis_options"][
                        "selected_options"
                    ]
                ]
    ...
    async def handle_app_mention(self, event, say):
        ...
        try:
            ...
            if "create task" in text or "new task" in text:
                await say(
                    "I'll help you create a task! Use `/ai-crm task create` for the full form."
                )
            elif "help" in text:
                ...
                await say(
                    "Hi! I'm the AI-CRM bot. Use `/ai-crm help` to see what I can do!"
                )
    ...
    async def fetch_agents_list(self) -> List[Dict]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.ai_crm_base_url}/agents", headers=self.ai_crm_headers
            ) as response:
    ...
    def format_task_confirmation_blocks(self, task_result: Dict) -> List[Dict]:
        ...
        return [
            ...
            },
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": f"*Title:* {task_result.get('title', 'N/A')}",
                },
                "accessory": {
                    "type": "button",
...
class SlackNotificationService:
    ...
    async def send_task_assignment_notification(
        self, channel: str, task: Dict, assignee_slack_id: str
    ):
        ...
        blocks = [
            ...
            },
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": f"<@{assignee_slack_id}> has been assigned a new task:",
                },
            },
            {
                "type": "section",
    ...
    async def send_task_status_update(
        ...
        message = (
            f"{status_emoji.get(new_status, '📌')} **Task Update**: {task['title']}\n"
        )
        message += (
            f"Status changed from *{old_status}* to *{new_status}* by <@{updated_by}>"
        )
    ...
    async def send_pm_analysis_result(
        self, channel: str, task_title: str, analysis_result: Dict
    ):
        ...
        if analysis_result.get("risk_factors"):
            risk_text = "\n".join(
                f"• {risk}" for risk in analysis_result["risk_factors"]
            )
            blocks.append(
                {
                    "type": "section",
...
async def main():
    ...
    parser.add_argument(
        "--port", type=int, default=3000, help="HTTP port (for http mode)"
    )
