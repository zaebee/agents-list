...
class GitHubIntegration:
    ...
    async def create_task_from_issue(self, issue: Dict, repository: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            task_data = {
                ...
                "priority": self.determine_priority_from_labels(issue.get('labels', [])),
                "source": "github",
            ...
            return {"created": True, "task_id": result.get('task_id'), "agent": result.get('assigned_agent')}
            
        except Exception as e:
            self.logger.error(f"Error creating task from issue: {e}")
            return {"created": False, "error": str(e)}
    
    async def create_task_from_pr(self, pull_request: Dict, repository: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"created": True, "task_id": result.get('task_id'), "reviewer": result.get('assigned_agent')}
    ...
    def format_pr_description(self, pull_request: Dict, repository: Dict) -> str:
        ...
        description += f"**Files Changed**: {pull_request.get('changed_files', 'N/A')}\n"
    ...
    async def get_repository_config(self, repo_full_name: str) -> Dict:
        ...
        try:
            ...
            self.logger.info(f"No configuration found for {repo_full_name}, using defaults")
    ...
    async def add_github_comment(self, repo_full_name: str, issue_number: int, comment: str, is_pr: bool = False):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status == 201:
                    ...
                    raise Exception(f"AI-CRM API error: {response.status} - {error_text}")
    
    async def update_ai_crm_task_status(self, task_id: str, status: str, metadata: Dict = None) -> Dict:
    ...
    async def handle_push_event(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(f"Push event: {repository.get('full_name')} - {len(commits)} commits")
            ...
            for task_ref in task_refs:
                try:
                    ...
                    result = await self.update_ai_crm_task_status(
                        task_ref['task_id'],
                        'completed' if task_ref['action'] in ['fixes', 'closes', 'resolves'] else 'in_progress',
                        {
    ...
    async def create_deployment_tasks(self, release: Dict, repository: Dict) -> Dict[str, Any]:
