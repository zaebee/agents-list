# AWS Infrastructure for AI-CRM System
# Production-ready deployment with ECS Fargate, RDS, and auto-scaling
...
terraform {
  ...
  backend "s3" {
    ...
    bucket = "ai-crm-terraform-state"
    key    = "ai-crm/terraform.tfstate"
    region = "us-east-1"
    encrypt = true
    dynamodb_table = "ai-crm-terraform-locks"
  }
}
...
resource "aws_secretsmanager_secret" "db_password" {
  ...
  description = "Database password for AI-CRM"
...
output "ecr_repositories" {
  ...
  value = {
    ...
    cli      = aws_ecr_repository.cli.repository_url
  }
}
