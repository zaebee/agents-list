...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if len(recent_requests) >= rate_limit:
            logger.warning(f"Rate limit exceeded for {client_key}: {len(recent_requests)}/{rate_limit}")
            return JSONResponse(
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        request_id = hashlib.md5(f"{time.time()}{request.client.host if request.client else 'unknown'}".encode()).hexdigest()[:16]
...
class InputValidationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        for param, value in request.query_params.items():
            if self._scan_for_malicious_content(value):
                logger.warning(f"Malicious content detected in query param '{param}': {value}")
                return JSONResponse(
...
def setup_security_middleware(app):
    ...
    app.add_middleware(RateLimitMiddleware, default_rate_limit=1000)  # 1000 requests per hour default
