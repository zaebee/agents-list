...
from database import get_db, User, AuditLog, AuthSession
from database import UserRole, SubscriptionTier, AccountStatus
from auth import (
    get_current_user, get_current_active_user, require_roles, require_subscription_tier,
    require_feature_access, log_auth_event, check_feature_access,
...
async def update_user_profile(
    ...
    try:
        ...
        await log_auth_event(
            db, current_user.id, "PROFILE_UPDATE", "SUCCESS", request,
            f"Profile updated for user: {current_user.username}", risk_score=1
        )
        
        return {
            "message": "Profile updated successfully",
            "requires_verification": profile_data.email and profile_data.email != current_user.email
        }
...
async def get_user_sessions(
    ...
    ).order_by(AuthSession.last_used.desc()).all()
...
async def revoke_session(
    ...
    try:
        ...
        await log_auth_event(
            db, current_user.id, "SESSION_REVOKE", "SUCCESS", request,
            f"Session {session_id} revoked by user: {current_user.username}", risk_score=1
        )
...
async def get_user_features(
    ...
    features = db.query(SubscriptionFeature).filter(
...
async def get_monthly_usage(
    ...
    task_feature = db.query(SubscriptionFeature).filter(
    ...
    tier_limits = {
        ...
        SubscriptionTier.ENTERPRISE: task_feature.enterprise_limit if task_feature else None
    }
...
async def update_user_admin(
    ...
    try:
        ...
        if update_data.role is not None:
            ...
            changes.append(f"role changed from {old_role.value} to {update_data.role.value}")
        ...
        if update_data.subscription_tier is not None:
            ...
            changes.append(f"subscription changed from {old_tier.value} to {update_data.subscription_tier.value}")
        ...
        if update_data.account_status is not None:
            ...
            changes.append(f"account status changed from {old_status.value} to {update_data.account_status.value}")
        ...
        # Log admin action
        await log_auth_event(
            db, current_user.id, "ADMIN_USER_UPDATE", "SUCCESS", request,
...
async def delete_user_admin(
    ...
    try:
        ...
        # Log admin action
        await log_auth_event(
            db, current_user.id, "ADMIN_USER_DELETE", "SUCCESS", request,
