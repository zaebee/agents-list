...
async def register_user(
    ...
    if not password_validation["is_valid"]:
        await log_auth_event(
            ...
            "FAILURE",
            request,
            f"Weak password for user: {user_data.username}",
            risk_score=2,
        )
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    try:
        ...
        await log_auth_event(
            ...
            "ERROR",
            request,
            f"Registration error for {user_data.username}: {str(e)}",
            risk_score=5,
        )
        raise HTTPException(
...
async def verify_email(
    ...
    if not user:
        await log_auth_event(
            ...
            "FAILURE",
            request,
            f"Invalid verification token: {verification_data.token}",
            risk_score=4,
        )
        raise HTTPException(
...
async def login_user(
    ...
    if not user or not verify_password(login_data.password, user.hashed_password):
        await log_auth_event(
            ...
            "FAILURE",
            request,
            f"Invalid credentials for: {login_data.username_or_email}",
            risk_score=6,
        )
        raise HTTPException(
    ...
    if not user.is_active:
        await log_auth_event(
            ...
            "FAILURE",
            request,
            f"Inactive account login attempt: {user.username}",
            risk_score=4,
        )
        raise HTTPException(
    ...
    if user.account_status == AccountStatus.SUSPENDED:
        await log_auth_event(
            ...
            "FAILURE",
            request,
            f"Suspended account login attempt: {user.username}",
            risk_score=7,
        )
        raise HTTPException(
    ...
    try:
        ...
        access_token_jti = (
            ...
            if token_payload
            else str(secrets.token_urlsafe(16))
        )
...
async def reset_password(
    ...
    if not user:
        await log_auth_event(
            ...
            "FAILURE",
            request,
            f"Invalid password reset token: {reset_data.token}",
            risk_score=6,
        )
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def change_password(
    ...
    if not verify_password(
        ...
        await log_auth_event(
            ...
            "FAILURE",
            request,
            f"Incorrect current password for user: {current_user.username}",
            risk_score=5,
        )
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
