...
AGENT_TOOL_RESTRICTIONS = {
    ...
    'language_specialists': {
        'agents': ['python-pro', 'javascript-pro', 'typescript-pro', 'golang-pro', 'rust-pro', 
                  'java-pro', 'csharp-pro', 'cpp-pro', 'c-pro', 'php-pro', 'scala-pro', 
                  'elixir-pro', 'sql-pro'],
    ...
    'architects': {
        'agents': ['backend-architect', 'architect-reviewer', 'cloud-architect', 'graphql-architect'],
        'tools': ['core', 'execution', 'web', 'task', 'plan']
    },
    
    # Frontend and UI - need core tools plus execution
    'frontend': {
        'agents': ['frontend-developer', 'ui-ux-designer', 'mobile-developer', 'ios-developer', 'unity-developer'],
        'tools': ['core', 'execution', 'web', 'task']
    },
    
    # Infrastructure and operations - need full system access
    'infrastructure': {
        'agents': ['devops-troubleshooter', 'deployment-engineer', 'terraform-specialist', 
                  'network-engineer', 'incident-responder', 'database-admin', 'database-optimizer'],
    ...
    'data_ai': {
        'agents': ['ai-engineer', 'ml-engineer', 'mlops-engineer', 'data-engineer', 'data-scientist',
                  'prompt-engineer'],
    ...
    'documentation': {
        'agents': ['docs-architect', 'api-documenter', 'reference-builder', 'tutorial-engineer',
                  'content-marketer', 'mermaid-expert'],
        'tools': ['core', 'web', 'task']
    },
    
    # Business and support - mainly web research and analysis
    'business': {
        'agents': ['business-analyst', 'sales-automator', 'customer-support', 'legal-advisor',
    ...
    'quality': {
        'agents': ['test-automator', 'debugger', 'error-detective', 'performance-engineer'],
        'tools': ['core', 'execution', 'web', 'task', 'process']
    },
    ...
    # Specialized tools
    'specialized': {
        'agents': ['payment-integration', 'legacy-modernizer', 'minecraft-bukkit-pro', 'dx-optimizer',
                  'context-manager'],
        'tools': ['core', 'execution', 'web', 'task']
    }
}
...
def update_agent_tools(agent_file: Path) -> bool:
    ...
    try:
        ...
        print(f"✅ Updated {agent_file.stem} ({category}) with tools: {', '.join(tools)}")
        return True
...
def main():
    ...
    for category, config in AGENT_TOOL_RESTRICTIONS.items():
        ...
        print(f"   Agents: {', '.join(config['agents'][:3])}{'...' if len(config['agents']) > 3 else ''}")
    
    proceed = input(f"\nProceed with updating {len(agent_files)} agent files? (y/N): ").strip().lower()
    if proceed != 'y':
