...
# Project imports
from models import (
    TaskCreateRequest, TaskUpdateRequest, TaskStatus, TaskPriority,
    PMAnalysisRequest, RoutingContext, TaskComplexity
)
from exceptions import CRMError, TaskNotFoundError, AgentNotFoundError
from crm_service import create_crm_service
from agent_routing import RoutingStrategy
...
class ModernCLI:
        ...
        try:
            if self.console:
                with Progress(
                    ...
                    task = progress.add_task("Initializing AI-CRM service...", total=None)
    ...
    async def create_task_interactive(self, args):
        ...
        try:
            # Get task details
            title = args.title or (Prompt.ask("Task title") if HAS_RICH else input("Task title: "))
            description = args.description or (Prompt.ask("Task description (optional)", default="") if HAS_RICH else input("Task description (optional): "))
            ...
            if not args.priority:
                if HAS_RICH:
                    ...
                    priority = input("Priority (low/medium/high/urgent) [medium]: ") or "medium"
            else:
            ...
            if not args.no_pm_analysis and not args.quiet:
                ...
                if self.console:
                    with Progress(
                        ...
                        task = progress.add_task("Analyzing task complexity...", total=None)
    ...
    def _display_pm_analysis(self, analysis):
        ...
        if self.console:
            ...
            table = Table(title="🎯 PM Agent Analysis", show_header=True, header_style="bold magenta")
            ...
            if analysis.success_criteria:
                success_panel = Panel(
                    "\n".join(f"• {criteria}" for criteria in analysis.success_criteria),
    ...
    def _display_tasks_table(self, tasks):
        ...
        table = Table(title=f"📋 Tasks ({len(tasks)})", show_header=True, header_style="bold magenta")
    ...
    def _display_task_detail(self, task):
        ...
        # Metadata
        if task.metadata and (task.metadata.risk_factors or task.metadata.success_criteria):
            metadata_text = ""
    ...
    async def suggest_agents(self, args):
        ...
        try:
            ...
            if self.console:
                ...
                table = Table(title="🤖 Agent Suggestions", show_header=True, header_style="bold magenta")
                ...
                for i, suggestion in enumerate(response.suggestions, 1):
                    workload_color = "green" if suggestion.workload_factor < 1.2 else "yellow" if suggestion.workload_factor < 1.5 else "red"
                    ...
                    table.add_row(
                        ...
                        suggestion.reasoning[:37] + "..." if len(suggestion.reasoning) > 40 else suggestion.reasoning
                    )
                ...
                for i, suggestion in enumerate(response.suggestions, 1):
                    print(f"{i}. {suggestion.agent_name} ({suggestion.confidence:.1f}% confidence)")
    ...
    async def show_system_stats(self, args):
        ...
        try:
            ...
            if self.console:
                ...
                overview_text += f"[bold]Success Rate:[/bold] {stats.success_rate:.1%}\n"
                overview_text += f"[bold]System Health:[/bold] {stats.system_health.title()}\n"
                ...
                if stats.most_active_agents:
                    agents_text = "\n".join(f"• {agent}" for agent in stats.most_active_agents[:5])
                    agents_panel = Panel(
                        agents_text,
                ...
                if stats.average_completion_time_hours:
                    print(f"Average Completion: {stats.average_completion_time_hours:.1f} hours")
...
Examples:
  ...
  %(prog)s stats
        ...
        create_parser.add_argument("--priority", choices=[p.value for p in TaskPriority], help="Task priority")
        create_parser.add_argument("--owner", help="Assign to specific agent")
        create_parser.add_argument("--no-pm-analysis", action="store_true", help="Skip PM Gateway analysis")
        create_parser.add_argument("--no-auto-assign", action="store_true", help="Skip automatic agent assignment")
        ...
        list_parser.add_argument("--offset", type=int, default=0, help="Skip first N tasks")
        list_parser.add_argument("--simple", action="store_true", help="Simple output format")
        ...
        view_parser.add_argument("--include-comments", action="store_true", help="Include task comments")
        view_parser.set_defaults(func=cli.view_task)
        
        # Suggest command
        suggest_parser = subparsers.add_parser("suggest", help="Get agent suggestions")
        suggest_parser.add_argument("--description", required=True, help="Task description to analyze")
        suggest_parser.add_argument("--limit", type=int, default=5, help="Maximum suggestions")
        ...
        health_parser.set_defaults(func=lambda args: cli.service.health_check() if cli.service else print("Service not initialized"))
