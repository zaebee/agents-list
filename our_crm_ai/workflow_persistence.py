...
from models import Task, Agent, TaskComplexity, TaskPriority
            ...
            for workflow_file in workflow_files:
                try:
                    ...
                    if file_time < cutoff_time:
                        ...
                        if (workflow_data and 
                            workflow_data.get('status') in ['completed', 'cancelled']):
                            
                            workflow_file.unlink()
                            cleaned_count += 1
                            self.logger.debug(f"Cleaned up old workflow: {workflow_file.stem}")
                
                except Exception as e:
                    self.logger.warning(f"Failed to cleanup workflow file {workflow_file}: {e}")
    ...
    async def save_workflow(self, workflow_id: str, workflow_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                """, (
                    workflow_id, title, description, status, priority,
                    complexity, estimated_hours, workflow_json
                ))
...
class WorkflowPersistenceManager:
    ...
    async def complete_workflow(self, task_id: str, completion_data: Dict[str, Any]) -> bool:
        ...
        return await self.update_workflow_status(task_id, 'completed', completion_updates)
    ...
    async def cancel_workflow(self, task_id: str, reason: str) -> bool:
        ...
        return await self.update_workflow_status(task_id, 'cancelled', cancellation_updates)
...
def create_file_persistence_manager(
    storage_dir: str = "workflow_storage",
    enable_caching: bool = True
