...
from pm_agent_gateway import PMAgentGateway, TaskPriority
...
class BacklogAnalyzer:
    ...
    def __init__(self):
        ...
        self.value_criteria = {
            ...
            'competitive_advantage': {'pm_agent': 4, 'monetization': 3, 'ui': 2, 'docs': 1}
        }
    ...
    def analyze_task(self, task: Dict) -> BacklogTask:
        ...
        pm_analysis = self.pm_gateway.create_managed_task(task['title'], task['description'])
    ...
    def print_analysis_report(self):
        ...
        for i, task in enumerate(analyzed_tasks, 1):
            ...
            print(f"   💰 Value: {task.business_value.name} | ⚡ Effort: {task.effort.name}")
        ...
        critical_tasks = [t for t in analyzed_tasks if t.business_value == BusinessValue.CRITICAL]
        if critical_tasks:
            print("🚨 IMMEDIATE FOCUS:")
            for task in critical_tasks[:2]:
                print(f"  • {task.title} - {task.strategic_impact}")
        
        quick_wins = [t for t in analyzed_tasks 
                     if t.priority_score > 2 and t.effort.value <= 2]
        if quick_wins:
            print("\n⚡ QUICK WINS:")
