...
from models import Task, TaskCreateRequest, TaskUpdateRequest, TaskStatus, TaskPriority, TaskMetadata
from exceptions import APIError, TaskNotFoundError, ConfigurationError
from config_manager import ConfigurationManager, load_config
from repositories import YouGileTaskRepository, create_task_repository
...
class YouGileIntegrationTester:
    ...
    def __init__(self, config_path: str = "config_enhanced.json", api_key: Optional[str] = None):
    ...
    async def test_task_retrieval(self, task: Task):
        ...
        try:
            ...
            if retrieved_task:
                # Verify task data
                success = (
                    retrieved_task.id == task.id and
                    retrieved_task.title == task.title and
                    retrieved_task.assigned_agent == task.assigned_agent
                )
                
                self.log_test_result(
                    "Task Retrieval",
                    success,
                    f"Retrieved task: {retrieved_task.id}"
                )
    ...
    async def test_task_update(self, task: Task):
        ...
        try:
            ...
            success = (
                updated_task.title != task.title and
                "[UPDATED]" in updated_task.title
            )
            
            self.log_test_result(
                "Task Update",
                success,
                f"Updated task: {updated_task.id}"
            )
            return updated_task
    ...
    async def test_task_status_transitions(self, task: Task):
        ...
        try:
            # Move to In Progress
            in_progress_task = await self.repository.move_task(task.id, TaskStatus.IN_PROGRESS)
            ...
            self.log_test_result(
                "Status Transitions",
                True,
                f"Successfully transitioned: TODO → IN_PROGRESS → DONE"
            )
            return done_task
    ...
    async def test_task_listing(self):
        ...
        try:
            ...
            todo_tasks = await self.repository.list_tasks(status=TaskStatus.TODO, limit=5)
            done_tasks = await self.repository.list_tasks(status=TaskStatus.DONE, limit=5)
    ...
    async def test_crm_service_integration(self):
        ...
        try:
            ...
            success = (
                ...
                response.task is not None and
                response.pm_analysis is not None and
                len(response.agent_suggestions) > 0
            )
...
async def main():
    ...
    parser.add_argument("--config", default="config_enhanced.json", help="Configuration file path")
    parser.add_argument("--api-key", help="YouGile API key (overrides environment variable)")
