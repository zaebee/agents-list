...
from models import Agent, Task, TaskPriority, TaskComplexity, AgentSuggestion
...
class WorkloadBalancer:
    ...
    async def calculate_workload_score(
        ...
        try:
            ...
            complexity_weight = self.complexity_weights.get(context.task_complexity, 2.0)
            ...
            self.logger.warning(f"Failed to calculate workload score for {agent.name}: {e}")
    ...
    async def predict_completion_time(
        ...
        try:
            ...
            # Adjust for task complexity
            complexity_multiplier = self.complexity_weights.get(context.task_complexity, 2.0)
...
class ContextAnalyzer:
    ...
    def __init__(self):
        ...
        self.task_patterns = {
            ...
            'new_feature': ['business-analyst', 'backend-architect', 'frontend-developer'],
            'infrastructure': ['cloud-architect', 'devops-troubleshooter', 'network-engineer'],
    ...
    def _classify_task_type(self, description: str) -> str:
        ...
        if any(word in desc_lower for word in ['bug', 'fix', 'error', 'broken']):
            return 'bug_fix'
        elif any(word in desc_lower for word in ['feature', 'implement', 'add', 'create']):
            return 'new_feature'
        elif any(word in desc_lower for word in ['deploy', 'infrastructure', 'server', 'cloud']):
            return 'infrastructure'
        elif any(word in desc_lower for word in ['data', 'analysis', 'report', 'metrics']):
    ...
    def _assess_collaboration_needs(self, description: str) -> Dict[str, Any]:
        ...
        collaboration_indicators = {
            'full_stack': any(word in desc_lower for word in ['full stack', 'end-to-end', 'complete']),
            'cross_functional': any(word in desc_lower for word in ['frontend and backend', 'ui and api']),
            'review_needed': any(word in desc_lower for word in ['review', 'approve', 'validate']),
            'integration': any(word in desc_lower for word in ['integrate', 'connect', 'sync'])
        }
    ...
    def _assess_technical_complexity(self, description: str) -> Dict[str, int]:
        ...
        if any(word in desc_lower for word in ['architecture', 'design pattern', 'scalable']):
            complexity_indicators['architectural'] = 3
        
        if any(word in desc_lower for word in ['algorithm', 'optimization', 'performance']):
    ...
    def calculate_context_score(
        ...
        if urgency_indicators and context.task_priority in [TaskPriority.HIGH, TaskPriority.URGENT]:
            score += 0.1
...
class SmartAgentRouter:
    ...
    async def route_task(
        ...
        try:
            ...
            if strategy == RoutingStrategy.BEST_MATCH:
                ...
                return await self._route_priority_aware(agents, context, task_description)
            elif strategy == RoutingStrategy.CONTEXT_AWARE:
                return await self._route_context_aware(agents, context, task_description)
    ...
    async def _route_priority_aware(
        ...
        for agent, suggestion in agents:
            ...
            workload_score = await self.workload_balancer.calculate_workload_score(agent, context)
            ...
            if context.task_priority in [TaskPriority.HIGH, TaskPriority.URGENT]:
                ...
                composite_score = (capability_score * 0.4 + workload_score * 0.6) * priority_multiplier
            else:
                # For lower priority, balance capability and workload
                composite_score = (capability_score * 0.7 + workload_score * 0.3) * priority_multiplier
    ...
    async def _route_context_aware(
        ...
        for agent, suggestion in agents:
            ...
            deadline_factor = 1.0
            if context.deadline:
                hours_until_deadline = (context.deadline - datetime.utcnow()).total_seconds() / 3600
            ...
            if composite_score > best_score and availability_score > 0:
                best_score = composite_score
                best_agent = (
                    agent, suggestion, capability_score, 
                    workload_score, availability_score, context_score, priority_bonus
                )
        
        if not best_agent:
            raise AgentError("No suitable agent found")
        
        agent, suggestion, cap_score, work_score, avail_score, ctx_score, pri_bonus = best_agent
    ...
    async def get_routing_recommendations(
        ...
        for strategy in strategies:
            try:
                ...
                self.logger.warning(f"Failed to get recommendation for {strategy.value}: {e}")
