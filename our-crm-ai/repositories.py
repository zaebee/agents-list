...
class AgentRepository(BaseRepository):
    ...
    async def update_agent_workload(
        self, agent_name: str, workload_delta: int
    ) -> Agent:
    ...
    async def update_agent_stats(
        self, agent_name: str, success: bool, completion_time_hours: float
    ) -> Agent:
            ...
            for attempt in range(self.max_retries + 1):
                try:
                    ...
                    if response.status_code == 200 or response.status_code == 201:
                        ...
                        error = create_api_exception(
                            response.status_code, "API request failed", response_data
                        )
    ...
    async def create_task(self, task_request: TaskCreateRequest) -> Task:
        ...
        try:
            ...
            if task_request.assigned_agent:
                ...
                state_id = ai_owner_config.get("states", {}).get(
                    task_request.assigned_agent
                )

                if not sticker_id or not state_id:
                    raise TaskValidationError(
                        f"Agent '{task_request.assigned_agent}' not configured in stickers"
                    )
    ...
    def _convert_api_task_to_model(self, api_data: dict[str, Any]) -> Task:
        ...
        if ai_owner_config:
            ...
            if sticker_id and sticker_id in stickers:
                ...
                for agent_name, agent_state_id in ai_owner_config.get(
                    "states", {}
                ).items():
        ...
        return Task(
            ...
            created_at=datetime.fromisoformat(
                api_data["created_at"].replace("Z", "+00:00")
            )
            if api_data.get("created_at")
            else None,
            updated_at=datetime.fromisoformat(
                api_data["updated_at"].replace("Z", "+00:00")
            )
...
class InMemoryAgentRepository(AgentRepository):
    ...
    async def update_agent_stats(
        ...
        new_response_time = (
            current_response_time * (1 - alpha) + completion_time_hours * alpha
        )
        ...
        try:
            ...
            if "yougile" in config_data:
                ...
                legacy_config_data = {
                    ...
                    "api_base_url": yougile_config.get(
...
async def create_task_repository(
    api_key: str, config: CRMConfiguration
) -> TaskRepository:
...
async def create_configuration_repository(
