...
class PMAgentGateway:
    ...
    def create_workflow(
        self, task_analysis: TaskAnalysis, title: str, description: str
    ) -> AgentWorkflow:
        ...
        if any(word in text for word in ["data", "analysis", "report", "metrics"]):
            ...
            word in text for word in ["infrastructure", "deploy", "server", "cloud"]
        ):
            template_key = "infrastructure_setup"

        # Get base workflow template
        base_workflow = self.workflow_templates.get(
            template_key, self.workflow_templates["full_stack_feature"]
        )
        ...
        for step in base_workflow:
            ...
            custom_step["duration_hours"] *= complexity_multiplier[
        ...
        return AgentWorkflow(
            ...
            primary_agent=task_analysis.required_agents[0]
            if task_analysis.required_agents
            else "general-purpose",
            supporting_agents=task_analysis.required_agents[1:]
    ...
    def recommend_task_priority(
        ...
        if any(
            word in text
            for word in ["urgent", "critical", "emergency", "outage", "down", "broken"]
        ):
        ...
        if any(
            word in text
            for word in ["feature", "enhancement", "improvement", "optimize"]
        ):
    ...
    def create_managed_task(
        ...
        if analysis.complexity in [TaskComplexity.COMPLEX, TaskComplexity.EPIC]:
            print("🔄 Complex task detected - recommending decomposition")
    ...
    def _calculate_progress_score(self, column_name: str) -> float:
        ...
        progress_map = {"To Do": 0.0, "In Progress": 0.5, "Done": 1.0}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="PM Agent Gateway - Intelligent task coordination"
    )
    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # Analyze command
    analyze_parser = subparsers.add_parser(
        "analyze", help="Analyze a task and provide recommendations"
    )
    ...
    analyze_parser.add_argument(
        "--auto-assign", action="store_true", help="Auto-assign to recommended agent"
    )
    ...
    decompose_parser = subparsers.add_parser(
        "decompose", help="Break down complex task into subtasks"
    )
    ...
    try:
        ...
        if args.command == "analyze":
            result = pm_gateway.create_managed_task(
                args.title, args.description, args.auto_assign
            )
