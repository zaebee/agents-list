...
class AgentSystemConfig(BaseModel):
    ...
    def validate_enabled_agents(cls, v, values):
        if "agent_configs" in values:
            ...
            if missing:
                raise ValueError(
                    f"Enabled agents not found in agent_configs: {missing}"
                )
        return v
    ...
    def validate_runtime_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                and not self._config.ai_provider.api_key
            ):
                if not os.getenv(f"{self.env_prefix}AI_API_KEY"):
                    issues.append(
                        f"AI_API_KEY environment variable required for {ai_provider}"
                    )
    ...
    def get_effective_config_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ai_provider": {
                ...
                "has_api_key": bool(
                    self._config.ai_provider.api_key
                    or os.getenv(f"{self.env_prefix}AI_API_KEY")
                ),
            },
            "yougile": {
