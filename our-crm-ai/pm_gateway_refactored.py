    ...
    def cleanup_old_workflows(self, days: int = 30):
        ...
        for task_id, workflow in self.workflows.items():
            created_at = datetime.fromisoformat(
                workflow.get("created_at", "1970-01-01")
            )
...
class TaskAnalyzer:
    ...
    def suggest_required_agents(self, title: str, description: str) -> list[str]:
        ...
        try:
            ...
            suggestions = self.agent_selector.suggest_agents(
                combined_text, max_suggestions=5
            )
            return (
                [s["agent"] for s in suggestions[:3]]
                if suggestions
...
class WorkflowEngine:
    ...
    def select_workflow_template(
        self, title: str, description: str, complexity: TaskComplexity
    ) -> str:
        ...
        if any(
            word in text
            for word in ["data", "analysis", "report", "metrics", "analytics"]
        ):
            return "data_analysis"
        elif any(
            word in text
            for word in ["infrastructure", "deploy", "server", "cloud", "devops"]
        ):
    ...
    def create_workflow(
        ...
        return {
            ...
            "supporting_agents": required_agents[1:]
...
class PMAgentGatewayRefactored:
    ...
    async def analyze_task(self, request: PMAnalysisRequest) -> PMAnalysisResult:
        ...
        try:
            ...
            complexity = self.task_analyzer.analyze_complexity(
                request.title, request.description
            )
            priority = self.task_analyzer.analyze_priority(
                request.title, request.description
            )
            estimated_hours = self.task_analyzer.estimate_effort(complexity)
            required_agents = self.task_analyzer.suggest_required_agents(
                request.title, request.description
            )
            risk_factors = self.task_analyzer.identify_risk_factors(
                request.title, request.description
            )
            success_criteria = self.task_analyzer.generate_success_criteria(
                request.title, request.description
            )
            ...
            subtasks = []
            recommended_agent = (
                required_agents[0] if required_agents else "general-purpose"
            )
            ...
            if complexity in [TaskComplexity.COMPLEX, TaskComplexity.EPIC]:
                ...
                self.logger.info(
                    f"🔄 Complex task - recommending {len(subtasks)} subtasks"
                )

            # Generate recommendation
            if task_type == "direct_assignment":
                recommendation = (
                    f"Assign to {recommended_agent} - estimated {estimated_hours} hours"
                )
            else:
                recommendation = (
                    f"Break into {len(subtasks)} subtasks for better management"
                )
    ...
    def create_managed_task(
        ...
        try:
            ...
            request = PMAnalysisRequest(
                title=title, description=description, auto_assign=auto_assign
            )
                ...
                if loop.is_running():
                    ...
                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        future = executor.submit(
                            asyncio.run, self.analyze_task(request)
                        )
            ...
            return {
                ...
                "subtasks": [],
                "recommendation": "Assign to general-purpose - estimated 8 hours",
                "task": {"title": title, "description": description},
            }
