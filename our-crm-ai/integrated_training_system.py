...
from agent_training_framework import (
    AgentTrainingPipeline, TrainingPhase, PerformanceMetric, 
    TrainingDataCollector, PerformanceEvaluator, ContinuousLearningEngine
)
from training_data_manager import TrainingDataCurator, DataSource
from performance_optimizer import AgentOptimizer, OptimizationStrategy, PerformanceMonitor
...
class IntegratedTrainingSystem:
    ...
    def _initialize_agent_status(self):
        ...
        all_agents = list(AGENT_KEYWORDS.keys()) + ['pm-agent-gateway', 'agent-selector', 'backlog-analyzer']
    ...
    def _determine_agent_priority(self, agent_name: str) -> TrainingPriority:
        ...
        high_priority_agents = [
            'python-pro', 'javascript-pro', 'database-optimizer', 
    ...
    def _setup_training_schedules(self):
        ...
        logging.info(f"Scheduled training for {len(self.config.training_schedule)} agent patterns")
    ...
    def _schedule_agent_training(self, agent_name: str):
        ...
        if (agent_name in [item['agent_name'] for item in self.training_queue] or 
            agent_name in self.active_training_sessions):
    ...
    def _handle_performance_alert(self, alert_data: Dict):
        ...
        logging.warning(f"Performance alert: {alert_data['metric']} = {alert_data['current_value']}")
    ...
    def _identify_agents_for_alert(self, alert_data: Dict) -> List[str]:
        ...
        if 'memory' in metric:
            return [agent for agent in self.agent_status.keys() 
            ...
            return [agent for agent, status in self.agent_status.items() 
    ...
    def _schedule_emergency_training(self, agent_name: str, alert_data: Dict):
        ...
        logging.warning(f"Emergency training scheduled for {agent_name} due to {alert_data['metric']} alert")
    ...
    async def _training_orchestrator(self):
        ...
        while True:
            try:
                ...
                if (len(self.active_training_sessions) < self.config.max_concurrent_training and 
                    self.training_queue):
    ...
    async def _start_agent_training(self, agent_name: str, training_item: Dict):
        ...
        try:
            ...
            insights = self.training_pipeline.continuous_learning.get_learning_insights(agent_name)
    ...
    async def _emergency_optimize_agent(self, agent_name: str, alert_context: Dict) -> Dict[str, Any]:
    ...
    async def _handle_training_completion(self, session_id: str):
        ...
        try:
            # Update agent status
            self.agent_status[agent_name].current_phase = None
            self.agent_status[agent_name].training_score = self._calculate_training_score(
                session_data['results']
            )
            
            # Calculate performance trend
            self.agent_status[agent_name].performance_trend = self._calculate_performance_trend(
            ...
            self.agent_status[agent_name].recommendations = self._generate_agent_recommendations(
                agent_name, session_data['results']
            )
            ...
            logging.info(f"Training completed for {agent_name} (score: {self.agent_status[agent_name].training_score:.2f})")
            
        except Exception as e:
            logging.error(f"Error handling training completion for {agent_name}: {e}")
            self.agent_status[agent_name].issues.append(f"Post-training error: {str(e)}")
        finally:
    ...
    def _calculate_performance_trend(self, agent_name: str, training_results: Dict[str, Any]) -> str:
    ...
    def _generate_agent_recommendations(self, agent_name: str, training_results: Dict[str, Any]) -> List[str]:
    ...
    async def _process_continuous_learning(self):
        ...
        for agent_name in self.agent_status.keys():
            try:
                ...
                logging.error(f"Error processing continuous learning for {agent_name}: {e}")
    
    def _process_agent_feedback(self, agent_name: str):
        """Process new feedback for an agent."""
        # Mock feedback processing
        feedback_score = 0.6 + (hash(agent_name + str(datetime.now().minute)) % 40) / 100
        
        # Update training score based on feedback
        current_score = self.agent_status[agent_name].training_score
        updated_score = (current_score * 0.9) + (feedback_score * 0.1)  # Weighted average
        ...
        if feedback_score < 0.4:
            self._schedule_emergency_training(agent_name, {
    ...
    def _collect_system_metrics(self):
        ...
        self.system_metrics.update({
            ...
            'healthy_agents': len([s for s in self.agent_status.values() if s.training_score > 0.7]),
            'average_training_score': statistics.mean([s.training_score for s in self.agent_status.values()]),
            'agents_needing_training': len([s for s in self.agent_status.values() 
    ...
    def _check_system_health(self):
        ...
        if metrics.get('healthy_agents', 0) < metrics.get('total_agents', 1) * 0.8:
            logging.warning("System health warning: Less than 80% of agents are healthy")
        
        if metrics.get('average_training_score', 0) < 0.6:
            logging.warning("System health warning: Low average training score")
        
        if metrics.get('agents_needing_training', 0) > metrics.get('total_agents', 1) * 0.3:
    ...
    def _update_agent_priorities(self):
        ...
        critical_agents = [name for name, status in self.agent_status.items() 
                          if status.priority == TrainingPriority.CRITICAL]
        
        logging.debug(f"Agent priorities updated. Critical agents: {len(critical_agents)}")
    ...
    def get_system_dashboard(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'agent_status_summary': {
                'total_agents': len(self.agent_status),
                'by_priority': {
                    priority.name: len([s for s in self.agent_status.values() if s.priority == priority])
                    for priority in TrainingPriority
                },
                'by_trend': {
                    trend: len([s for s in self.agent_status.values() if s.performance_trend == trend])
                    for trend in ['improving', 'stable', 'declining', 'unknown']
                },
                'training_scores': {
                    'excellent': len([s for s in self.agent_status.values() if s.training_score > 0.9]),
                    'good': len([s for s in self.agent_status.values() if 0.7 < s.training_score <= 0.9]),
                    'acceptable': len([s for s in self.agent_status.values() if 0.5 < s.training_score <= 0.7]),
                    'poor': len([s for s in self.agent_status.values() if s.training_score <= 0.5])
                }
            },
            'training_activity': {
                'active_sessions': len(self.active_training_sessions),
                'queue_length': len(self.training_queue),
                'recent_completions': len([h for h in self.training_history 
                                         if h.get('completed_at', datetime.min) > datetime.now() - timedelta(hours=24)])
            },
            'top_performing_agents': [
                {'name': name, 'score': status.training_score}
                for name, status in sorted(self.agent_status.items(), 
                                         key=lambda x: x[1].training_score, reverse=True)[:5]
            ],
    ...
    def get_agent_details(self, agent_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'current_session': next((s for s in self.active_training_sessions.values() 
                                   if s['agent_name'] == agent_name), None),
...
def main():
    ...
    start_parser = subparsers.add_parser('start', help='Start the integrated training system')
    ...
    retrain_parser = subparsers.add_parser('emergency-retrain', help='Emergency retrain agent')
    retrain_parser.add_argument('agent_name', help='Agent name')
    retrain_parser.add_argument('--reason', required=True, help='Reason for emergency retraining')
    ...
    try:
        if args.command == 'start':
            ...
            print(f"Active training sessions: {metrics.get('active_training_sessions', 0)}")
            ...
            print(f"Average training score: {metrics.get('average_training_score', 0):.2f}")
            ...
            if success:
                ...
                print(f"❌ Failed to schedule emergency retraining for {args.agent_name}")
