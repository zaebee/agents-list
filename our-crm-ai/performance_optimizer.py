...
from agent_training_framework import PerformanceMetric, AgentPerformanceData
        ...
        for metric_name, value in metrics.items():
            ...
            if alert_triggered:
                alert_data = {
                    ...
                    'threshold': threshold,
                    'timestamp': datetime.now(),
                    'severity': self._calculate_alert_severity(metric_name, value, threshold)
                }
                
                self._trigger_alert(alert_data)
    
    def _calculate_alert_severity(self, metric_name: str, value: float, threshold: float) -> str:
...
class AgentOptimizer:
    ...
    def optimize_agent(self, agent_name: str, strategy: OptimizationStrategy,
                      current_metrics: Dict[str, float]) -> Dict[str, Any]:
        """Optimize an agent using specified strategy."""
        
        logging.info(f"Starting optimization for {agent_name} with strategy {strategy.value}")
        ...
        for technique in techniques:
            ...
            try:
                ...
                improvement = self._calculate_improvement(best_metrics, optimized_metrics, strategy)
                ...
                if improvement > 0:
                    ...
                    logging.info(f"{technique.value} improved {strategy.value} by {improvement:.2%}")
        ...
        optimization_results['optimization_success'] = len(optimization_results['techniques_applied']) > 0
    ...
    def _get_optimization_techniques(self, agent_name: str, strategy: OptimizationStrategy) -> List[OptimizationTechnique]:
    ...
    def _apply_optimization_technique(self, agent_name: str, technique: OptimizationTechnique,
        ...
        if technique == OptimizationTechnique.CACHING:
            ...
            optimized_metrics['response_time'] = current_metrics.get('response_time', 2.0) * 0.6
            
        elif technique == OptimizationTechnique.CONTEXT_PRUNING:
            # Context pruning improves response time and resource efficiency
            optimized_metrics['response_time'] = current_metrics.get('response_time', 2.0) * 0.8
            optimized_metrics['memory_usage'] = current_metrics.get('memory_usage', 0.5) * 0.7
            
        elif technique == OptimizationTechnique.PROMPT_OPTIMIZATION:
            # Prompt optimization improves accuracy and response time
            optimized_metrics['accuracy'] = min(current_metrics.get('accuracy', 0.7) * 1.1, 1.0)
            optimized_metrics['response_time'] = current_metrics.get('response_time', 2.0) * 0.9
            
        elif technique == OptimizationTechnique.BATCHING:
            # Batching improves throughput
            optimized_metrics['throughput'] = current_metrics.get('throughput', 10.0) * 1.3
            
        elif technique == OptimizationTechnique.PARALLEL_PROCESSING:
            # Parallel processing improves throughput and response time
            optimized_metrics['throughput'] = current_metrics.get('throughput', 10.0) * 1.5
            optimized_metrics['response_time'] = current_metrics.get('response_time', 2.0) * 0.7
            
        elif technique == OptimizationTechnique.MEMORY_OPTIMIZATION:
            # Memory optimization reduces resource usage
            optimized_metrics['memory_usage'] = current_metrics.get('memory_usage', 0.5) * 0.6
    ...
    def _calculate_all_improvements(self, before_metrics: Dict[str, float],
    ...
    def _store_optimization_history(self, optimization_results: Dict[str, Any]):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """, (
                ...
                json.dumps([t['technique'] for t in optimization_results['techniques_applied']]),
    ...
    def run_performance_benchmark(self, agent_name: str, test_scenarios: List[Dict]) -> PerformanceBenchmark:
        ...
        for strategy in [OptimizationStrategy.RESPONSE_TIME, OptimizationStrategy.ACCURACY]:
            optimization_result = self.optimize_agent(agent_name, strategy, baseline_metrics)
        ...
        for metric in baseline_metrics:
            if metric in optimized_metrics:
                ...
                if baseline_val != 0:
                    improvement_percentage[metric] = ((optimized_val - baseline_val) / baseline_val) * 100
    ...
    def _collect_baseline_metrics(self, agent_name: str, test_scenarios: List[Dict]) -> Dict[str, float]:
        ...
        if 'database' in agent_name:
            ...
            baseline_metrics['response_time'] *= 0.8  # Frontend agents are typically faster
    ...
    def _store_benchmark_results(self, benchmark: PerformanceBenchmark):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """, (
                ...
                json.dumps([tech.value for tech in benchmark.optimization_techniques]),
    ...
    def start_ab_test(self, agent_name: str, test_name: str, 
                     control_version: str, treatment_version: str,
                     sample_size: int = 100, confidence_level: float = 0.95) -> str:
        ...
        logging.info(f"Started A/B test {experiment_id} for {agent_name}: {control_version} vs {treatment_version}")
        
        return experiment_id
    
    def add_ab_test_data(self, experiment_id: str, version: str, metrics: Dict[str, float]):
        ...
        if version == experiment.control_version:
            ...
            raise ValueError(f"Unknown version {version} for experiment {experiment_id}")
        ...
        control_samples = len(list(experiment.control_metrics.values())[0]) if experiment.control_metrics else 0
        treatment_samples = len(list(experiment.treatment_metrics.values())[0]) if experiment.treatment_metrics else 0
        
        if control_samples >= experiment.sample_size and treatment_samples >= experiment.sample_size:
    ...
    def _analyze_ab_test(self, experiment_id: str):
        ...
        # Analyze each metric
        for metric_name in set(experiment.control_metrics.keys()) | set(experiment.treatment_metrics.keys()):
            ...
            if pooled_std > 0:
                t_stat = abs(treatment_mean - control_mean) / (pooled_std * ((1/len(control_values) + 1/len(treatment_values)) ** 0.5))
        ...
        significant_metrics = [r for r in results.values() if r['significant'] and r['improvement'] > 0]
        experiment.statistical_significance = len(significant_metrics) > 0
        
        # Store results in database
        self._store_ab_test_results(experiment)
        
        logging.info(f"A/B test {experiment_id} completed: {len(significant_metrics)} significant improvements")
    ...
    def _store_ab_test_results(self, experiment: ABTestExperiment):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """, (
                ...
                json.dumps({k: list(v) for k, v in experiment.control_metrics.items()}),
                json.dumps({k: list(v) for k, v in experiment.treatment_metrics.items()}),
    ...
    def get_optimization_recommendations(self, agent_name: str,
        ...
        for issue in performance_issues:
            ...
            if issue['issue'] == 'slow_response':
                ...
                recommendations.append({
                    ...
                    'techniques': [
                        OptimizationTechnique.PROMPT_OPTIMIZATION.value
                    ],
...
def main():
    ...
    # Optimize command
    optimize_parser = subparsers.add_parser('optimize', help='Optimize agent performance')
    optimize_parser.add_argument('--agent', required=True, help='Agent name')
    optimize_parser.add_argument('--strategy', choices=[s.value for s in OptimizationStrategy], 
                                required=True, help='Optimization strategy')
    ...
    try:
        if args.command == 'optimize':
            ...
            recommendations = optimizer.get_optimization_recommendations(args.agent, current_metrics)
