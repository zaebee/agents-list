...
class TrainingDataCollector:
    ...
    def collect_pm_gateway_decisions(
        self,
        task_title: str,
        task_description: str,
    ...
    def _flush_buffer(self, agent_name: str = None):
        ...
        agents_to_flush = (
            [agent_name] if agent_name else list(self.collection_buffer.keys())
        )
...
class PerformanceEvaluator:
    ...
    def get_agent_performance_summary(
        ...
        for metric_name, metric_data in self.metric_history[agent_name].items():
            recent_data = [
                data.value for data in metric_data if data.timestamp >= cutoff_date
            ]
    ...
    def compare_agents(
        ...
        for agent_name in agent_names:
            if (
                agent_name in self.metric_history
                and metric.value in self.metric_history[agent_name]
            ):
                recent_values = [
                    data.value
                    for data in list(self.metric_history[agent_name][metric.value])[
                        -10:
                    ]
                ]
                if recent_values:
...
class ContinuousLearningEngine:
    ...
    def _check_retraining_triggers(self, agent_name: str):
        ...
        summary = self.performance_evaluator.get_agent_performance_summary(
            agent_name, days_back=7
        )
        ...
        for metric_name, stats in summary.items():
            ...
            if stats["trend"] < -0.5 and stats["sample_count"] >= 5:
                ...
                logging.info(
                    f"Agent {agent_name} showing declining {metric_name}: trend {stats['trend']}"
                )
            ...
            if metric_name == "user_satisfaction" and stats["average"] < 0.6:
                retraining_needed = True
                logging.info(
                    f"Agent {agent_name} user satisfaction below threshold: {stats['average']}"
                )
    ...
    def get_learning_insights(self, agent_name: str) -> dict[str, Any]:
        ...
        performance_summary = self.performance_evaluator.get_agent_performance_summary(
            agent_name
        )
    ...
    def _analyze_failure_patterns(
        ...
        top_word_patterns = sorted(
            common_words.items(), key=lambda x: x[1], reverse=True
        )[:5]
        top_context_patterns = sorted(
        ...
        return {
            ...
            "failure_rate": len(low_performance_data) / len(training_data)
            if training_data
            else 0,
        }
    ...
    def _generate_improvement_recommendations(
        ...
        for metric_name, stats in performance_summary.items():
            if stats["trend"] < -0.3:
                recommendations.append(
                    f"Focus on improving {metric_name} - showing decline trend"
                )
            ...
            if metric_name == "accuracy" and stats["average"] < 0.7:
                recommendations.append(
                    "Improve accuracy through additional domain-specific training"
                )
        ...
        if failure_patterns["count"] > 0:
            ...
            if failure_rate > 0.3:
                recommendations.append(
                    "High failure rate detected - review task assignment criteria"
                )
            ...
            if failure_patterns["patterns"].get("common_failure_words"):
                ...
                recommendations.append(
                    f"Focus training on tasks involving: {words_str}"
                )
        ...
        if agent_name in AGENT_KEYWORDS:
            ...
            recommendations.append(
                f"Strengthen expertise in: {', '.join(agent_keywords[:3])}"
            )
    ...
    def _assess_training_data_quality(
        ...
        avg_quality = statistics.mean(
            [self.data_collector._calculate_quality_score(dp) for dp in training_data]
        )
...
class AgentTrainingPipeline:
    ...
    def start_training_session(
        self, agent_name: str, phase: TrainingPhase, training_data_limit: int = None
    ) -> str:
        ...
        performance_before = self.performance_evaluator.get_agent_performance_summary(
            agent_name
        )
        ...
        # Log training start
        logging.info(
            f"Started {phase.value} training session {session_id} for {agent_name} "
    ...
    def complete_training_session(
        ...
        session.performance_after = (
            self.performance_evaluator.get_agent_performance_summary(
        ...
        logging.info(
            f"Completed training session {session_id} for {session.agent_name}"
        )
        return True
    ...
    def run_comprehensive_training(self, agent_name: str) -> dict[str, Any]:
        ...
        baseline_performance = self.performance_evaluator.get_agent_performance_summary(
            agent_name
        )
        ...
        for phase in training_phases:
            ...
            if session_id:
                ...
                # Complete session
                success = self.complete_training_session(
                    session_id, notes=f"Automated {phase.value} training"
                )
        ...
        # Final assessment
        final_performance = self.performance_evaluator.get_agent_performance_summary(
            agent_name
        )
    ...
    def _determine_training_phases(
        ...
        specialization_level = self.agent_metadata[agent_name].get(
    ...
    def _generate_system_insights(self) -> dict[str, Any]:
        ...
        for metric in PerformanceMetric:
            ...
            insights["top_performers"][metric.value] = dict(
                list(top_agents.items())[:5]
            )

        # Identify training needs
        insights["training_needs"] = (
            self.performance_evaluator.identify_training_needs()
        )
        ...
        for agent_name in all_agents:
            agent_summary = self.performance_evaluator.get_agent_performance_summary(
                agent_name
            )
            if agent_summary:
        ...
        return {
            "summary": {
                ...
                "active_training_sessions": len(
                    [s for s in recent_sessions if s.status == "running"]
                ),
                "completed_sessions_7d": len(
                    [s for s in recent_sessions if s.status == "completed"]
                ),
            ...
            "recent_training_sessions": [
                asdict(session) for session in recent_sessions[-10:]
            ],
...
def main():
    ...
    train_parser = subparsers.add_parser(
        "train", help="Train specific agent or all agents"
    )
    train_parser.add_argument(
        "--agent", help='Agent name to train (or "all" for all agents)'
    )
    train_parser.add_argument(
        "--phase", choices=[p.value for p in TrainingPhase], help="Training phase"
    )

    # Status command
    status_parser = subparsers.add_parser(
        "status", help="Show training status and performance"
    )
    status_parser.add_argument("--agent", help="Specific agent to show status for")

    # Dashboard command
    dashboard_parser = subparsers.add_parser(
        "dashboard", help="Show training dashboard data"
    )

    # Collect command
    collect_parser = subparsers.add_parser(
        "collect", help="Collect training data from task"
    )
    ...
    collect_parser.add_argument(
        "--task-description", required=True, help="Task description"
    )
    collect_parser.add_argument("--response", required=True, help="Agent response")
    collect_parser.add_argument(
        "--rating", type=float, required=True, help="Success rating (0-1)"
    )
    ...
    try:
        if args.command == "train":
            if args.agent == "all":
                ...
                print(
                    f"System health score: {results['system_insights']['system_health']['overall_score']:.2f}"
                )
            ...
            if args.agent:
                insights = pipeline.continuous_learning.get_learning_insights(
                    args.agent
                )
                performance = (
                    pipeline.performance_evaluator.get_agent_performance_summary(
                        args.agent
                    )
                )
                ...
                print(
                    f"Training data quality: {insights['training_data_quality']['overall']:.2f}"
                )
                ...
                if performance:
                    ...
                    for metric, stats in performance.items():
                        print(
                            f"  {metric}: {stats['average']:.2f} (trend: {stats['trend']:+.2f})"
                        )
                ...
                print(
                    f"Active training sessions: {summary['active_training_sessions']}"
                )
                ...
                print(
                    f"Healthy agents: {system_health['healthy_agents']}/{system_health['total_evaluated']}"
                )
