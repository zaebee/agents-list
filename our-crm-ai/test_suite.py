...
class TestModels(unittest.TestCase):
    ...
    def test_task_validation_errors(self):
        ...
        with self.assertRaises(ValueError):
            Task(
                id="1", description="d", status=TaskStatus.TODO, title=""
            )  # Empty title should fail

        with self.assertRaises(ValueError):
            Task(
                id="1", description="d", status=TaskStatus.TODO, title="x" * 201
    ...
    def test_task_create_request(self):
        ...
        request = TaskCreateRequest(
            title="New Feature", description="Implement new feature", priority="high"
        )
...
class TestTaskAnalyzer(unittest.TestCase):
    ...
    def test_complexity_analysis(self):
        ...
        simple_complexity = self.analyzer.analyze_complexity(
            "Fix button color", "Change the submit button color to blue"
        )
        self.assertEqual(simple_complexity, TaskComplexity.SIMPLE)

        # Complex task
        complex_complexity = self.analyzer.analyze_complexity(
            "Redesign system architecture", "Platform migration with new architecture"
        )
    ...
    def test_priority_analysis(self):
        ...
        urgent_priority = self.analyzer.analyze_priority(
            "Production down", "Critical system outage affecting all users"
        )
        self.assertEqual(urgent_priority, TaskPriority.URGENT)

        # Low priority task
        low_priority = self.analyzer.analyze_priority(
            "Update documentation", "Refresh user guide with latest features"
        )
...
class TestAsyncComponents:
    ...
    async def test_workflow_persistence_manager(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            success = await manager.save_workflow_state(
                "task-123", "Test Task", "Test description", workflow_data
            )
            assert success
    ...
    async def test_enhanced_agent_selector(self):
        ...
        )  # Disable semantic for testing
        await selector.initialize()

        # Test agent suggestions
        suggestions = await selector.suggest_agents(
            "Create a Python API endpoint with database integration", max_suggestions=3
        )
    ...
    async def test_learning_system(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            python_score = learning_system.get_agent_performance_score(
                "python-pro", "Fix authentication bug in Python service"
            )
    ...
    async def test_smart_agent_router(self):
        ...
        suggestions = [
            AgentSuggestion(
                agent_name="python-pro", confidence=85.0, reasoning="Python expert"
            ),
            AgentSuggestion(
                agent_name="backend-architect",
                confidence=75.0,
                reasoning="Architecture match",
            ),
            AgentSuggestion(
...
class TestConfigurationManager(unittest.TestCase):
    ...
    def test_config_loading(self):
        ...
        test_config = {
            ...
            "yougile": {
                ...
                "board_id": "test-board",
                "columns": {"To Do": "col-1", "In Progress": "col-2", "Done": "col-3"},
            },
            "agents": {
    ...
    def test_environment_overrides(self):
        ...
        try:
            ...
            minimal_config = {
                "ai_provider": {"provider": "openai", "model": "gpt-3.5-turbo"},
                "yougile": {
                    "project_id": "test",
                    "board_id": "test",
                ...
                "agents": {
                    ...
                    "agent_configs": {"general-purpose": {"name": "general-purpose"}},
                },
            }
    ...
    def test_runtime_validation(self):
        ...
        # Create valid config
        valid_config = {
            "ai_provider": {"provider": "openai", "model": "gpt-4"},
            "yougile": {
                "project_id": "test-project",
                "board_id": "test-board",
                "columns": {"To Do": "1", "In Progress": "2", "Done": "3"},
            ...
            "agents": {
                ...
                "agent_configs": {"test-agent": {"name": "test-agent"}},
            },
        }
...
class TestRunner:
    ...
    def generate_test_report() -> Dict[str, Any]:
        ...
        if total_errors > 0:
            report["recommendations"].append(f"Resolve {total_errors} test errors")
...
if __name__ == "__main__":
    ...
    print(
        f"Async Tests: {'✅ PASSED' if report['summary']['async_tests_success'] else '❌ FAILED'}"
    )
