...
async def test_mistral_integration():
    ...
    mistral_agents = [
        agent_id for agent_id, config in framework.agents.items()
    ...
    try:
        ...
        if health_status:
            print(f"✅ Mistral agent '{test_config.agent_name}' is healthy and responsive")
        else:
    ...
    try:
        result = await framework.execute_task(test_task)
        
        if result.status == TaskStatus.COMPLETED:
            print("✅ Task executed successfully!")
            print(f"   Result length: {len(result.result.get('result', ''))} characters")
            print(f"   Tokens used: {result.tokens_used}")
            print(f"   Cost: ${result.cost:.6f}")
            print(f"   Quality score: {result.quality_score:.2f}")
            
            # Show a snippet of the result
            result_text = result.result.get('result', '')[:200]
    ...
    if len(mistral_agents) > 1:
        ...
        for i, agent_id in enumerate(mistral_agents[:2]):  # Test first 2
            ...
            try:
                result = await framework.execute_task(simple_task)
                if result.status == TaskStatus.COMPLETED:
                    print(f"     ✅ Success - {result.tokens_used} tokens, ${result.cost:.6f}")
                else:
    ...
    total_tasks = len([t for t in framework.completed_tasks.values() if framework.agents[t.agent_id].provider == "mistral"])
    total_cost = sum([t.cost for t in framework.completed_tasks.values() if framework.agents[t.agent_id].provider == "mistral"])
    total_tokens = sum([t.tokens_used for t in framework.completed_tasks.values() if framework.agents[t.agent_id].provider == "mistral"])
